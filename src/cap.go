...
func (sub *CapSubcommand) SetFlags(fs *flag.FlagSet) {
	...
	fs.BoolVar(&sub.truncateNames, "truncate", false, "Truncate column names if too long")
...
func Cap(inputCsv AbstractInputCsv, names []string, truncateNames bool, defaultName string) {
	...
	if numColumns < numNames && !truncateNames {
		fmt.Fprintf(os.Stderr, "Must specify --truncate if there are fewere columns than column names provided")
		os.Exit(1)
	}

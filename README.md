...
```shell
gocsv cap --names NAMES [--truncate] [--default-name DEFAULT_NAME] FILE
```

Arguments:

- `--names` A comma-separated list of names to add as headers to each column.
- `--truncate` [optional] If there are fewer columns than the number of names provided by `--names`, drop the extra column names.
...
* The number of names provided by `--names` is greater than the number of columns and `--truncate` is not specified, or
